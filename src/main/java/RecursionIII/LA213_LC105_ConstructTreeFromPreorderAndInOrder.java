...
public class LA213_LC105_ConstructTreeFromPreorderAndInOrder {
    public TreeNode buildTree(int[] preorder, int[] inorder) {
        ...
        for (int i = 0; i < inorder.length; i++) {
            ...
            // 记录每个treenode.key作为map的key；
            //  对应所在的index作为map的value
        }
    ...
    private TreeNode helper(int[] preorder, Map<Integer, Integer> indexMap, int inLeft, int inRight, int preLeft, int preRight) {
        if (inLeft > inRight) {
            // 注意这里base case 一定要是inLeft > inRight，不能等于！否则的话当前root的左边只有一个a的话，应该root.left = a的，但是如果可以等于，相当于只有一个元素的时候就return null了，那么就相当于root.left = null了
            return null;
        }
